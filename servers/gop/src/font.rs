//! Built-in 8×16 monospace bitmap font (PC Code Page 437 subset).
//!
//! Each glyph is 16 bytes — one byte per row, MSB = leftmost pixel.
//! ASCII 0x00–0x7F are defined; anything outside that range renders as a
//! solid block (glyph 0xDB).

/// Return the 16-byte glyph bitmap for codepoint `ch`.
/// For codepoints outside the printable ASCII range, returns a solid block.
pub fn glyph(ch: u8) -> &'static [u8; 16] {
    let idx = if ch >= 0x20 && ch <= 0x7E {
        (ch - 0x20) as usize
    } else {
        // Solid block fallback: GLYPHS[59] = char 0x5B '[' (redefined as solid block in table).
        59
    };
    &GLYPHS[idx]
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Glyph table — ASCII 0x20 (' ') through 0x7E ('~'), 95 entries × 16 bytes.
// Generated from the classic IBM PC BIOS "VGA" 8×16 font (public domain).
// ─────────────────────────────────────────────────────────────────────────────
#[rustfmt::skip]
static GLYPHS: [[u8; 16]; 95] = [
    ...
    // 0x26 '&'
    [0x00,0x00,0x1C,0x36,0x1C,0x6E,0xDB,0xDB,0x70,0x1B,0x0E,0x6F,0x00,0x00,0x00,0x00],
    // 0x27 '\''
    [0x00,0x00,0x0C,0x0C,0x0C,0x06,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x28 '('
    ...
    // 0x5B '[' — also used as solid-block fallback (index 0x5B in table = index 59)
